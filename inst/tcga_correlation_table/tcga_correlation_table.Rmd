...
all <- data %>% 
  ...
  summarize(
    n = n(),
    `all pvalue` = cor.test(RP3, miR34a, method = "spearman")$p.value,
    `all rho` = cor.test(RP3, miR34a, method = "spearman")$estimate
  ) %>%
  mutate(
    `all pvalue` = format(`all pvalue`, scientific = TRUE, digits = 3),
...
p53 <- data %>%
  ...
  summarize(
    n = n(),
    pvalue = cor.test(RP3, miR34a, method = "spearman")$p.value,
...
table <- full_join(all, p53, by = "cancer_PAM50") %>%
  ...
  select(
    cancer, `all n`, `all rho`, `all pvalue`, `TP53wt n`, 
    `TP53wt rho`, `TP53wt pvalue`, `TP53mut n`, `TP53mut rho`, `TP53mut pvalue`
  )
