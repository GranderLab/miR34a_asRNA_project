...
all <- data %>% 
  ...
  summarize(
    n = n(),
    all_pvalue = signif(cor.test(RP3, miR34a, method = "spearman")$p.value, 3),
    all_R_estimate = signif(cor.test(RP3, miR34a, method = "spearman")$estimate, 3)
  )
...
p53 <- data %>%
  ...
  summarize(
    n = n(),
    pvalue = signif(cor.test(RP3, miR34a, method = "spearman")$p.value, 3),
    R_estimate = signif(cor.test(RP3, miR34a, method = "spearman")$estimate, 3)
  ) %>%
  ungroup() %>%
  gather(metric, value, -(cancer:TP53)) %>%
  unite(temp, TP53, metric) %>%
  spread(temp, value)

full_join(all, p53, by = "cancer")
```
