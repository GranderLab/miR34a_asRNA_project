...
p <- stats %>%
  ggplot(aes(gene_name, mean, color = fraction)) +
  geom_point(position = position_dodge(width = 1)) +
  facet_wrap(~`Biosample term name`, ncol = 1) +
  geom_errorbar(aes(ymax = sd_max, ymin = sd_min), width = 0, position = position_dodge(width = 1)) +
...
# path <- file.path("~/GitHub/miR34a_asRNA_project/inst", fileMap(type = "pdf")["figure2a"][[1]])
# ggsave(
#  plot = figure,
#  filename = path,
#  device = cairo_pdf,
