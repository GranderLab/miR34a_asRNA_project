...
nrCellLinesDown <- function() {
  dataUrl <- "https://github.com/GranderLab/miR34a_asRNA_project/raw/master/"
  url <- fileMap(type = "txt")["suppFigure3d"][[1]]
...
cellLinesInFinal <- function() {
  dataUrl <- "https://github.com/GranderLab/miR34a_asRNA_project/raw/master/"
  url <- fileMap(type = "rds")["suppFigure3d"][[1]]
...
data <- filter(data, start < 9245000) %>%
  mutate(stop = if_else(feature == "intron" & start != 9242375, 9245000, stop))
...
# path <- file.path("~/GitHub/miR34a_asRNA_project/inst", fileMap(type = "pdf")["suppFigure3d"][[1]])
# ggsave(
#   plot = g,
#   filename = path,
#   device = cairo_pdf,
