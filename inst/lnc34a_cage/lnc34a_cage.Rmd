...
gdata <- filter(gdata, start < 9245000) %>%
...
gdata <- bind_rows(gdata, pw) %>%
  mutate(gene = readr::parse_factor(gene, levels = c("Primers", "lnc34a", "miR34a asRNA")))
...
p1 <- ggplot(data = NULL) +
  ...
  geom_text(
    data = filter(gdata, feature == "primer"),
    aes(x = start + 10, y = 2.65, label = fill),
    size = 1.75,
    angle = 90,
    colour = "grey20"
  ) +
...
grobz <- lapply(list(p1, p2, p3), ggplotGrob)
g <- rbind(grobz[[2]], grobz[[3]], grobz[[1]], size = "last")
g <- resize_heights(g, unit(c(5, 1, 1.5), "null"))
#grid.draw(g)
