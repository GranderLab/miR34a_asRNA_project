...
#preform ddct analysis
groups <- c("Condition", "Treatment", "Biological Replicate", "Cell line", "gene")
data <- full_join(goi, select(hk, -gene), by = groups[groups != "gene"]) %>%
  technicalMeans(.) %>%
  dct(.) %>%
  ddct(., Treatment, 0, groups) %>%
  folds(.)
...
```{r, fig.align='center', fig.height=8, fig.width=12, eval=FALSE, message=FALSE, warning=FALSE}
#plot for Rmarkdown
