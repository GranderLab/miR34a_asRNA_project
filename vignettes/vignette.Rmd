...
groups <- c("Condition", "Treatment", "Biological Replicate", "Cell line")
data <- full_join(goi, select(hk, -gene), by = groups) %>%
  technicalMeans(.) %>%
  dct(.)


#ddct
data <- ddct(data, Treatment, 0, c("gene", "Condition", "Biological Replicate")) %>%
  folds(.)
