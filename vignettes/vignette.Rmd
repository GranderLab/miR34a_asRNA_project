...
ggplot(data = NULL) +
  ...
  geom_point(
    data = data,
    aes(x = Treatment, y = log2fold, colour = Condition),
    size = 4,
...
stats <- data %>%
  ...
  summarize(
    ...
    pValue = .calcP(`Cell line`, gene, value, data)
  ) %>%
  ungroup() %>%
  pFormat(.)
...
ggplot(data = NULL) +
  geom_violin(
    data = data,
      aes(x = construct, y = value, fill = gene),
      trim = TRUE,
      alpha = 0.3,
      color = NA,
      scale = "width",
  ...
  geom_point(
    data = data,
    aes(x = construct, y = value, colour = gene),
    size = 4,
    position = position_dodge(width = 0.9),
    stroke = 0
  ) +
  ...
  geom_linerange(
    data = stats,
    aes(x = construct, ymin = CI95l, ymax = CI95h, group = gene),
    colour = "black",
    position = position_dodge(width = 0.9),
    show.legend = FALSE
  ) +
  geom_point(
    data = stats,
    aes(x = construct, y = mean, group = gene),
    colour = "black",
    position = position_dodge(width = 0.9),
    show.legend = FALSE
  ) +
  geom_label(
    data = stats,
    aes(x = construct, y = CI95h + 1900, label = pFormat, group = gene),
    label.size = 0,
    label.padding = unit(0.01, "lines"),
    show.legend = FALSE,
    fill = "white",
...
ggplot(data = NULL) +
  geom_violin(
    data = data,
    aes(x = treatment, y = log2fold, fill = plotGOI),
    trim=TRUE,
    scale="width",
    alpha=0.3,
    colour=NA,
  ...
  geom_point(
    data = data,
    aes(x = treatment, y = log2fold, colour = plotGOI),
    size = 4,
    position = position_dodge(width = 0.9)
  ) +
  ...
  geom_linerange(
    data = stats,
    aes(x = treatment, ymin = CI95l, ymax = CI95h, group = plotGOI),
    colour = "black",
    position = position_dodge(width = 0.9),
    show.legend = FALSE
  ) +
  geom_point(
    data = stats,
    aes(x = treatment, y = mean, group = plotGOI),
    colour = "black",
    position = position_dodge(width = 0.9),
    show.legend = FALSE
  ) +
  geom_label(
    data = stats,
    aes(x = treatment, y = CI95h + 0.1, label = pFormat, group = plotGOI),
    label.size = 0,
    label.padding = unit(0.01, "lines"),
    show.legend = FALSE,
    position = position_dodge(width = 0.9),
...
ggplot(data = NULL)+
  geom_violin(
    data = data,
    aes(x = Condition, y = log2fold, group = interaction(Condition, GOI)),
    trim=TRUE,
    alpha=0.5,
    fill="grey",
    colour=NA,
  ...
  geom_point(
    data = data,
    aes(x = Condition, y = log2fold, colour = GOI, group = interaction(Condition, GOI)),
    size = 2, 
    alpha = 0.75,
    position = position_dodge(width = 0.9)
  ) +
  ...
  geom_linerange(
    data = stats,
    aes(x = Condition, ymin = CI95l, ymax = CI95h, group = interaction(Condition, GOI)),
    position = position_dodge(width = 0.9),
    colour = "grey25",
    show.legend = FALSE
  ) +
  geom_point(
    data = stats,
    aes(x = Condition, y=mean, group = interaction(Condition, GOI)),
    colour = "black",
    size = 1,
    position = position_dodge(width = 0.9),
    show.legend = FALSE
  ...
  geom_label(
    data = stats,
    aes(x = Condition, y = CI95h + 0.5, label = pFormat, group = interaction(Condition, GOI)),
    fill = "white",
    label.size = 0,
    label.padding = unit(0.01, "lines"),
    position = position_dodge(width = 0.9),
  ...
  guides(
    color = guide_legend(title = "Gene", override.aes = list(size = 4)))
```
...
ggplot(data = NULL) +
  geom_violin(
    data = data,
    aes(x = phase, y = value, fill = condition),
    scale = "width",
    alpha = 0.3, 
    weight = 10,
    trim = TRUE,
  ...
  geom_point(
    data = data,
    aes(x = phase, y = value, group = interaction(phase, condition), color = condition),
    position = position_dodge(width = 0.9),
    size = 2
  ) +
  facet_grid(
  ...
  geom_linerange(
    data = stats,
    aes(x = phase, ymin = CI95l, ymax = CI95h, group = interaction(phase, condition)),
    position = position_dodge(width = 0.9),
    colour = "grey25",
    show.legend = FALSE
  ) +
  ...
  geom_segment(
    data = stats,
    aes(x = x, y = y + 0.01, xend = xend, yend = y + 0.01),
    colour = "grey43",
    show.legend = FALSE
  ) +
  geom_label(
    data = stats,
    aes(x = phase, y = y + 0.02, label = pFormat),
    label.size = 0,
    label.padding = unit(0.01, "lines"),
    show.legend = FALSE,
    fill = "white",
...
ggplot(data = NULL) + 
  geom_ribbon(
    data = stats,
    aes(x = Time, ymin = CI95l, ymax = CI95h, group = `Cell line`, fill = `Cell line`), 
    alpha = 0.2,
    show.legend = TRUE
  ) +
  geom_linerange(
    data = stats,
    aes(x = Time, colour = `Cell line`, ymin = min, ymax = max), 
    position = position_dodge(width = 1),
    show.legend = FALSE
  ) + 
  geom_point(
    data = stats,
    aes(x = Time, y = median, group = `Cell line`, colour = `Cell line`), 
    position = position_dodge(width = 1),
    show.legend = FALSE
  ) +
...
#calculate percent of input
.percentOfInput <- function(data, techMean, Biological.Replicate, condition) {
...
data <- data %>%
  ...
  mutate(
    percentInput = .percentOfInput(data, techMean, `Biological Replicate`, condition),
    logPercentInput = log2(percentInput)
  ) %>%
  ungroup() %>%
  filter(sample != "input")

#calculate fold over control (mock)
.foldOverControl <- function(data, logPercentInput, Biological.Replicate) {
  bool1 <- data$`Biological Replicate` == Biological.Replicate
  bool2 <- data$condition == "mock"
  bool <- bool1 & bool2
  logPercentInput - pull(data, logPercentInput)[bool]
}

data <- data %>%
  group_by(`Biological Replicate`) %>%
  mutate(
    logfoldOverControl = .foldOverControl(data, logPercentInput, `Biological Replicate`),
    foldOverControl = 2^logfoldOverControl
  )
...
ggplot(data = NULL)+
  geom_violin(
    data = data,
    aes(x = condition, y = logfoldOverControl, fill = condition),
    trim = TRUE,
    alpha = 0.3,
    colour = NA,
    show.legend = FALSE
  ...
  geom_point( 
    data = data,
    aes(x = condition, y = logfoldOverControl, colour = condition),
    size = 4
  ) +
  geom_linerange(
    data = stats,
  ...
  geom_label(
    data = stats,
    aes(x = 1.5, y = CI95h + 0.35, label = pFormat),
    label.size = 0,
    label.padding = unit(0.01, "lines"),
    show.legend = FALSE,
    fill = "white",
